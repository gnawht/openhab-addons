...
/**
 * The {@link MessageRaw} is responsible for notifications
 *
 * @author Thomas Wang-Nielsen - Initial contribution
 */
public class MessageRaw {

    // protected static byte[] calcCRC16(byte[] buffer, int size) {
    // int CRC = 0x0000ffff;
    ...
    // byte[] result = ByteBuffer.allocate(4).putInt(CRC).array();
    // return result;
    // }

    protected static byte[] calcCRC16(byte[] buff, int buffLen) {
        int hi, lo, tmp;
        int crcSeed = 0xFFFF;
        lo = crcSeed & 0x0ff;
        hi = crcSeed >> 8 & 0x0ff;
        ...
        // CRC = ((CRC & 0x0000FF00) >> 8) | ((CRC & 0x000000FF) << 8);
        byte[] result = ByteBuffer.allocate(4).putInt(crc).array();
        byte[] resultCrc = ByteBuffer.allocate(2).array();
        resultCrc[0] = result[3];
        resultCrc[1] = result[2];
        return resultCrc;
    }
